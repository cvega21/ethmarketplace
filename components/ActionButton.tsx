...
const ActionButton = (props: IProps) => {
  ...
  return (
    ...
    : props.theme === 'dark' ?
      <Link href={`/${props.link}`} passHref={true}>
          <button className="bg-indigo-700 rounded-lg hover:bg-indigo-800 text-gray-100 hover:text-white font-medium text-xl py-2 px-8 my-4 shadow-indigo w-full transition-all duration-200 ease-in-out">
            {props.children}
          </button>
      </Link>
    : <></>
